...
function initP2P() {
	(function () {
        ...
        function removeRemotePlayer(id) {
            ...
            removeBodyAndConstraints(remotePlayers[id]);
			...
			try {
				switch (packetType) {
                    ...
                    case protocol.mob_create: {
                        ...
                        const [sides, off7] = BinaryProtocol.readUint8(view, off6);
                        const [category, off8] = BinaryProtocol.readUint32(view, off7);
                        const [mask, off9] = BinaryProtocol.readUint32(view, off8);
                        if (!mob.some(m => m.netId === netId)) {
                            ...
                            if (typeof spawn[typeName] === "function") {
                                ...
                                try {
                                    ...
                                    if (mob.length > beforeLen) {
                                        newMob = mob[beforeLen];
                                        if (mob.length > beforeLen + 1) {
                                            for (let i = beforeLen + 1; i < mob.length; i++) removeBodyAndConstraints(mob[i]);
                                            mob.splice(beforeLen + 1, mob.length - beforeLen - 1);
                                        }
                                    }
                                } catch (e) {
                                    if (mob.length > beforeLen) {
                                        for (let i = beforeLen; i < mob.length; i++) removeBodyAndConstraints(mob[i]);
                                        mob.splice(beforeLen, mob.length - beforeLen);
                                    }
                                }
                            }
                            if (newMob) {
                                ...
                                if (category) newMob.collisionFilter.category = category;
                                if (mask) newMob.collisionFilter.mask = mask;
                                makePuppetMob(newMob);
                            } else {
                                mobs.spawn(posX, posY, sides || 8, radius || 20, fill || "#000");
                                ...
                                if (fallbackMob) {
                                    ...
                                    if (category) fallbackMob.collisionFilter.category = category;
                                    if (mask) fallbackMob.collisionFilter.mask = mask;
                    ...
                    case protocol.mob_update: {
                        ...
                        const [fill, off11] = BinaryProtocol.readString(view, off10);
                        ...
                        } else if (!window.isLevelAuthority && mob.some(m => m.netId)) {
                            const now = performance.now();
                            if (now - _lastMobResyncRequest > 2000) {
                                _lastMobResyncRequest = now;
                                requestAllMobs(0);
                            }
                        }
                        break;
                    ...
                    case protocol.mob_death: {
                        const [netId, off1] = BinaryProtocol.readString(view, offset);
                        const idx = mob.findIndex(m => m.netId === netId);
                        if (idx !== -1 && mob[idx].alive) {
                            const target = mob[idx];
                            target.leaveBody = false;
                            target.isDropPowerUp = false;
                            target.alive = false;
                            removeBodyAndConstraints(target);
                            mob.splice(idx, 1);
                        }
                        break;
                    }
                    case protocol.mob_props: {
                        if (!remotePlayers[senderId]?.updateBlocks) break;
                        const [netId, off1] = BinaryProtocol.readString(view, offset);
                        const [propsJSON, off2] = BinaryProtocol.readString(view, off1);
                        const target = mob.find(m => m.netId === netId);
                        if (target && target.alive) {
                            try {
                                const props = JSON.parse(propsJSON);
                                for (const key of mobSyncedProps) {
                                    if (Object.prototype.hasOwnProperty.call(props, key)) target[key] = props[key];
                                }
                            } catch (e) {}
                        }
                        break;
        ...
        function sendBlockClaim(block) {
            ...
            off = BinaryProtocol.writeUint8(view, off, Math.min(255, (mobObj.vertices && mobObj.vertices.length) || 8));
            off = BinaryProtocol.writeUint32(view, off, (mobObj.collisionFilter && mobObj.collisionFilter.category) || cat.mob);
            off = BinaryProtocol.writeUint32(view, off, (mobObj.collisionFilter && mobObj.collisionFilter.mask) || (cat.player | cat.map | cat.body | cat.bullet | cat.mob));
            sendBlockData(buf);
        }
        ...
        function sendAllMobs(connection) {
            ...
            for (const mobObj of aliveMobs) {
                const sendTypeName = mobObj._syncTypeName || mobObj._mobTypeName;
                const typeEnc = new TextEncoder().encode(sendTypeName);
                const idEnc = new TextEncoder().encode(mobObj.netId);
                const fillEnc = new TextEncoder().encode(mobObj.fill || "#000");
                const buf = new ArrayBuffer(
                    1 + 1 +
                ...
                let off = BinaryProtocol.writeUint8(view, 0, protocol.mob_create);
                off = BinaryProtocol.writeUint8(view, off, clientId);
                off = BinaryProtocol.writeString(view, off, sendTypeName);
                off = BinaryProtocol.writeString(view, off, mobObj.netId);
                off = BinaryProtocol.writeFloat64(view, off, mobObj.position.x);
                off = BinaryProtocol.writeFloat64(view, off, mobObj.position.y);
                off = BinaryProtocol.writeFloat64(view, off, mobObj.radius || 20);
        ...
        for (const [name, fn] of Object.entries(m.skin)) {
            ...
            m.skin[name] = function(...args) {
                ...
                window.lastUsedSkin = name;
                sendSkin(name);
                return result;
            };
        }
        function sendSkin(skinName) {
            const encoder = new TextEncoder();
            const nameBytes = encoder.encode(skinName);

            const buffer = new ArrayBuffer(3 + nameBytes.length);
            const view = new DataView(buffer);

            let offset = BinaryProtocol.writeUint8(view, 0, protocol.player_skin);
            offset = BinaryProtocol.writeUint8(view, offset, clientId);
            offset = BinaryProtocol.writeUint8(view, offset, nameBytes.length);
        ...
        simulation.startGame = (isBuildRun = false, isTrainingRun = false) => {
            ...
            for (const playerKey in remotePlayers) {
                if (remotePlayers.hasOwnProperty(playerKey)) {
                    removeBodyAndConstraints(remotePlayers[playerKey]);
                    const idx = mob.findIndex(m => m.id === playerKey);
                    if (idx !== -1) mob.splice(idx, 1);
        ...
        level.start = function() {
            ...
            const staleMobs = mob.filter(m => !isRemotePlayerBody(m));
            for (const m of staleMobs) {
            ...
            if (!window.isLevelAuthority) {
                ...
                const toWipe2 = mob.filter(b => !isRemotePlayerBody(b));
        ...
        let _nextMobId = 1;
        let _lastMobResyncRequest = 0;
        let _inPuppetDoContext = false;
        const mobUpdateInterval = 100;
        function removeBodyAndConstraints(target) {
            if (!target) return;
            const attached = Composite.allConstraints(engine.world).filter(c => c.bodyA === target || c.bodyB === target);
            for (const c of attached) Composite.remove(engine.world, c);
        ...
        function wrapMobDeathForSync(mobObj) {
            if (!mobObj || mobObj._deathSyncWrapped || typeof mobObj.death !== 'function') return;
            mobObj._deathSyncWrapped = true;
            const origDeath = mobObj.death;
            mobObj.death = function(...args) {
                const result = origDeath.apply(this, args);
                if (window.isLevelAuthority && this.netId && connections.some(c => c.open)) {
                    sendMobDeath(this.netId);
                }
                return result;
            };
        }
        ...
        function makePuppetMob(mobObj) {
            ...
            mobObj.seePlayerByLookingAt = function() {
            ...
            mobObj.do = function() {
                _inPuppetDoContext = true;
                try {
                    origDo.call(this);
                } finally {
                    _inPuppetDoContext = false;
                }
        ...
        (function wrapSpawnFunctions() {
            ...
            const companionOnlyKeys = new Set(['shield', 'groupShield', 'orbital', 'spawnOrbitals']);
            const dispatcherOnlyKeys = new Set(['randomMob', 'randomSmallMob', 'randomLevelBoss', 'randomGroup', 'secondaryBossChance']);
            for (const key of Object.keys(spawn)) {
                if (typeof spawn[key] !== 'function') continue;
                if (skipKeys.has(key)) continue;
                const orig = spawn[key];
                spawn[key] = function(...args) {
                    if (_inPuppetDoContext) {
                        return undefined;
                    }
                    if (companionOnlyKeys.has(key) && !window.isLevelAuthority) {
                        return undefined;
                    }
                    ...
                    if (window.isLevelAuthority) {
                        ...
                        for (let i = 0; i < mob.length; i++) {
                            const mobObj = mob[i];
                            if (mobObj && !before.has(mobObj)) {
                                if (seenNew.has(mobObj)) {
                                    mob.splice(i, 1);
                                    i--;
                                    continue;
                                }
                        ...
                        if (!dispatcherOnlyKeys.has(key)) {
                            let isFirstNewMob = true;
                            for (const mobObj of mob) {
                                if (mobObj && !before.has(mobObj) && !mobObj.netId && !mobObj.isPowerUp && !isRemotePlayerBody(mobObj)) {
                                    const sentTypeName = isFirstNewMob ? key : `${key}#part`;
                                    isFirstNewMob = false;
                                    if (!mobObj._mobTypeName) mobObj._mobTypeName = key;
                                    mobObj._syncTypeName = sentTypeName;
                                    if (!window.isSpawningLevelBlocks) {
                                        mobObj.netId = `mob_${level.onLevel}_${_nextMobId++}`;
                                        mobObj.remoteMob = false;
                                        wrapMobDeathForSync(mobObj);
                                        sendMobCreate(mobObj, sentTypeName);
                                    }
                                }
                            }
                        }
        ...
        if (typeof powerUps !== 'undefined') {
            for (const key of Object.keys(powerUps)) {
                if (typeof powerUps[key] !== 'function') continue;
                const origPowerUpFn = powerUps[key];
                powerUps[key] = function(...args) {
                    const before = new Set(mob);
                    const result = origPowerUpFn.apply(this, args);
                    for (const mobObj of mob) {
                        if (mobObj && !before.has(mobObj) && !mobObj.isPowerUp) {
                            mobObj.isPowerUp = true;
                        }
                    }
                    return result;
                };
        ...
        const _origMobsSpawn = mobs.spawn;
        mobs.spawn = function(...args) {
            if (_inPuppetDoContext) return undefined;
            return _origMobsSpawn.apply(this, args);
        };
        ...
        level.start = function() {
            ...
            if (window.isLevelAuthority) {
                for (const mobObj of mob) {
                    if (!mobObj.netId && !mobObj.isPowerUp && !isRemotePlayerBody(mobObj)) {
                        mobObj.netId = `mob_${level.onLevel}_${_nextMobId++}`;
                ...
                if (connections.some(c => c.open)) {
                    for (const mobObj of mob) {
                        if (!isRemotePlayerBody(mobObj)) sendMobCreate(mobObj, mobObj._syncTypeName || mobObj._mobTypeName);
                    }
                }
            }
        };
        ...
        const mobSyncedProps = ['isInvulnerable', 'shield', 'isShielded', 'noseLength', 'tier', 'fireAngle', 'damageReduction', 'isUnblockable', 'laserArray'];
        const _lastSentProps = new WeakMap();
        function sendMobProps(mobObj) {
            if (!mobObj || !mobObj.netId) return;
            let last = _lastSentProps.get(mobObj);
            if (!last) { last = {}; _lastSentProps.set(mobObj, last); }
            const diff = {};
            let changed = false;
            for (const key of mobSyncedProps) {
                const val = mobObj[key];
                const t = typeof val;
                let ok = false;
                let comparable;
            ...
            if (!changed) return;
            const idEnc = new TextEncoder().encode(mobObj.netId);
            const propsEnc = new TextEncoder().encode(JSON.stringify(diff));
            const buf = new ArrayBuffer(1 + 1 + 2 + idEnc.length + 2 + propsEnc.length);
            const view = new DataView(buf);
            let off = BinaryProtocol.writeUint8(view, 0, protocol.mob_props);
            off = BinaryProtocol.writeUint8(view, off, clientId);
            off = BinaryProtocol.writeString(view, off, mobObj.netId);
            off = BinaryProtocol.writeString(view, off, JSON.stringify(diff));
            sendBlockData(buf);
        }
        setInterval(() => {
            ...
            for (const mobObj of mob) {
                if (mobObj && mobObj.alive && mobObj.netId && !isRemotePlayerBody(mobObj)) {
                    sendMobUpdate(mobObj);
                    sendMobProps(mobObj);
                }
            }
        }, mobUpdateInterval);
		...
		function spawnPlayer(xPos, yPos, id) {
				...
				function enter() {
					...
					if (!me.onGround) {
						...
						if (me.crouch) {
							...
							const momentum = me.velocity.y * me.mass;
            ...
            me.skin = {
                ...
                favicon() { //used to render the favicon, not actually in game
                    ...
                    me.fillColor = `hsl(${me.color.hue},${me.color.sat}%,${me.color.light}%)`
                    me.fillColorDark = `hsl(${me.color.hue},${me.color.sat}%,${me.color.light - 10}%)`
                    let grd = ctx.createLinearGradient(-30, 0, 30, 0);
                    grd.addColorStop(0, me.fillColorDark);
                    grd.addColorStop(1, me.fillColor);
                    me.bodyGradient = grd
                ...
                egg() {
                    me.isAltSkin = true
                    me.yOffWhen.stand = 52
                    me.yOffWhen.jump = 72
                    me.squirrelJump = 1.15;
                    me.setMovement()
                    ...
                    me.draw = function () {
                        if (powerUps.boost.endCycle > simulation.cycle) {
                            ...
                            ctx.save();
                            ctx.translate(me.pos.x, me.pos.y);
                            me.velocitySmooth = Vector.add(Vector.mult(me.velocitySmooth, 0.8), Vector.mult(player.velocity, 0.2))
                            ctx.rotate(Math.atan2(me.velocitySmooth.y, me.velocitySmooth.x))
                            ctx.beginPath();
                            const radius = 39
                            const mag = 14 * Vector.magnitude(me.velocitySmooth) + radius
                            ...
                            // const time = (powerUps.boost.endCycle - me.cycle) / powerUps.boost.duration
                        ...
                        ctx.stroke();
                        ctx.restore();
                        me.yOff = me.yOff * 0.75 + me.yOffGoal * 0.25; //smoothly move leg height towards height goal
                    }
                    me.drawLeg = function (stroke) {
                        if (me.angle2 > -Math.PI / 2 && me.angle2 < Math.PI / 2) {
                            me.flipLegs = 1;
                        ...
                        const hip = { x: me.hip.x - 5, y: me.hip.y + 5 }
                        const sub = Vector.sub(me.knee, hip)
                        ...
                        const foot = { x: me.foot.x - 10, y: me.foot.y - 15 }
                        ctx.save();
                        ctx.scale(me.flipLegs, 1); //leg lines
                        ctx.beginPath();
                        ...
                        ctx.quadraticCurveTo(me.foot.x - 30, me.foot.y + 12, me.foot.x + 13, me.foot.y + 3);
                        ctx.lineWidth = 1.5;
                        ctx.stroke();

                        //hip joint
                        ctx.beginPath();
                        ctx.arc(me.hip.x, me.hip.y - 2, 11, 0, 2 * Math.PI);
                        //knee joint
                        ctx.moveTo(me.knee.x + 3, me.knee.y);
                        ctx.arc(me.knee.x, me.knee.y, 3, 0, 2 * Math.PI);
                        //knee brace
                        ...
                        ctx.fillStyle = "#f6f6f6"//me.fillColor;
                        ctx.fill();
                        ctx.lineWidth = 1;
                        // ctx.strokeStyle = "#333"
                        ctx.stroke();
                ...
                mech() {
                    ...
                    me.yOffWhen.stand = 52
                    me.yOffWhen.jump = 72
                    me.coyoteCycles = 12 + 120 * me.tech.isCoyote
                    me.hardLandCDScale = 0.5
                    me.hardLanding = 250
                    me.squirrelFx = 1.4;
                    me.squirrelJump = 1.17;
                    me.setMovement()
                    ...
                    me.drawLeg = function (stroke) {
                        ...
                        ctx.moveTo(foot.x, foot.y);
                        ctx.quadraticCurveTo(me.foot.x - 30, me.foot.y + 12, me.foot.x + 13, me.foot.y + 3);
                        // me.lastOnGroundCycle + me.coyoteCycles > me.cycle

                        ctx.lineWidth = 1.5
                        ctx.stroke();
                        if (me.coyoteCycles > 30 && !me.onGround) {
                            ctx.lineWidth = 0.2 * Math.max(0, Math.min(3 * (me.cycle - me.lastOnGroundCycle), Math.min(120, me.lastOnGroundCycle + me.coyoteCycles - me.cycle)))
                            ctx.strokeStyle = "rgba(255, 255, 0, 0.3)"
                            ctx.stroke()
                        }
                        //hip joint
                        ctx.beginPath();
                        ctx.arc(me.hip.x, me.hip.y - 1, 11, 0, 2 * Math.PI);
                        //knee joint
                        ctx.moveTo(me.knee.x + 3, me.knee.y);
                        ctx.arc(me.knee.x, me.knee.y, 3, 0, 2 * Math.PI);
                        //knee brace
                ...
                polar() {
                    ...
                    // me.setFillColors();
                    me.fillColor = `hsl(${me.color.hue},${me.color.sat}%,${me.color.light}%)`
                    me.fillColorDark = `hsl(${me.color.hue},${me.color.sat}%,${me.color.light - 35}%)`
                    let grd = ctx.createLinearGradient(-30, 0, 30, 0);
                    grd.addColorStop(0, me.fillColorDark);
                    grd.addColorStop(0.7, me.fillColor);
                    // grd.addColorStop(1, me.fillColor);
                    me.bodyGradient = grd
                    ...
                    me.draw = function () {
                        ...
                        const diff = (me.lastKillCycle - me.cycle + me.tech.isDamageCooldownTime) / me.tech.isDamageCooldownTime
                        const color = diff < 0 ? "#fff" : "#aaa"
                        const hue = 220 + 20 * Math.sin(0.01 * me.cycle)
                        const colorInverse = diff < 0 ? `hsl(${hue}, 80%, 40%)` : "#fff"
                        me.drawLeg(color, colorInverse);
                        me.calcLeg(0, 0);
                        me.drawLeg(color, colorInverse);
                        ...
                        ctx.beginPath();
                        ctx.ellipse(me.pos.x, me.pos.y, 24, 18, 3.14 * Math.random(), 0, 2 * Math.PI)
                    ...
                    me.drawLeg = function (stroke, circles) {
                        // if (simulation.mouseInGame.x > me.pos.x) {
                        if (me.angle2 > -Math.PI / 2 && me.angle2 < Math.PI / 2) {
                            me.flipLegs = 1;
                        } else {
                            me.flipLegs = -1;
                        ...
                        ctx.moveTo(me.foot.x, me.foot.y);
                        if (me.onGround) {
                            ctx.lineTo(me.foot.x - 15, me.foot.y + 5);
                            ctx.moveTo(me.foot.x, me.foot.y);
                            ctx.lineTo(me.foot.x + 15, me.foot.y + 5);
                        } else {
                            ctx.lineTo(me.foot.x - 13, me.foot.y + 8);
                            ctx.moveTo(me.foot.x, me.foot.y);
                            ctx.lineTo(me.foot.x + 13, me.foot.y + 8);
                        }
                        ctx.lineWidth = 3;
                        ...
                        //hip joint
                        ctx.beginPath();
                        ctx.arc(me.hip.x, me.hip.y, 11, 0, 2 * Math.PI);
                        //knee joint
                        ctx.moveTo(me.knee.x + 5, me.knee.y);
                        ctx.arc(me.knee.x, me.knee.y, 5, 0, 2 * Math.PI);
                        //foot joint
                        ctx.moveTo(me.foot.x + 5, me.foot.y);
                        ctx.arc(me.foot.x, me.foot.y, 5, 0, 2 * Math.PI);
                ...
                strokeGap() {
                    ...
                    me.yOffWhen.stand = 52
                    me.yOffWhen.jump = 72
                    me.coyoteCycles = 11 + 120 * me.tech.isCoyote
                    ...
                    me.draw = function () {
                        if (powerUps.boost.endCycle > simulation.cycle) {
                            ...
                            ctx.beginPath();
                            const radius = 40
                            const mag = 9 * Vector.magnitude(me.velocitySmooth) + radius
                            ctx.arc(0, 0, radius, -Math.PI / 2, Math.PI / 2);
                            ctx.bezierCurveTo(-radius, radius, -radius, 0, -mag, 0); // bezierCurveTo(cp1x, cp1y, cp2x, cp2y, x, y)
                            ctx.bezierCurveTo(-radius, 0, -radius, -radius, 0, -radius);
                        ...
                        ctx.beginPath();
                        ctx.arc(0, 0, 30, 0, 2 * Math.PI);
                        // ctx.arc(0, 0, 30, me.angle2 + 1, me.angle2 - 1);
                        ctx.fillStyle = "#fff"//me.bodyGradient
                        ctx.fill();
                        ctx.beginPath();
                        const arc = 0.7 + 0.17 * Math.sin(me.cycle * 0.012)
                    ...
                    me.drawLeg = function (stroke) {
                        ...
                        //hip joint
                        ctx.beginPath();
                        ctx.arc(me.hip.x, me.hip.y, 8, 0, 2 * Math.PI);
                        //knee joint
                        ctx.moveTo(me.knee.x + 4, me.knee.y);
                        ctx.arc(me.knee.x, me.knee.y, 4, 0, 2 * Math.PI);
                        //foot joint
                        ctx.moveTo(me.foot.x + 4, me.foot.y + 1);
                        ctx.arc(me.foot.x, me.foot.y + 1, 4, 0, 2 * Math.PI);
                        ctx.fillStyle = me.fillColor;
                ...
                energy() {
                    ...
                    me.draw = function () {
                        if (powerUps.boost.endCycle > simulation.cycle) {
                            ...
                            ctx.beginPath();
                            const radius = 40
                            const mag = 10 * Vector.magnitude(me.velocitySmooth) + radius
                            ctx.arc(0, 0, radius, -Math.PI / 2, Math.PI / 2);
                            ctx.bezierCurveTo(-radius, radius, -radius, 0, -mag, 0); // bezierCurveTo(cp1x, cp1y, cp2x, cp2y, x, y)
                            ctx.bezierCurveTo(-radius, 0, -radius, -radius, 0, -radius);
                        ...
                        ctx.lineWidth = 15;
                        ctx.stroke();
                        ctx.fillStyle = 'hsl(184,100%,85%)' //me.fillColor; //"#9ff" //me.bodyGradient
                        ctx.fill();
                    ...
                    me.drawLeg = function (stroke) {
                        ...
                        //hip joint
                        ctx.beginPath();
                        ctx.arc(me.hip.x, me.hip.y, 10, 0, 2 * Math.PI);
                        //knee joint
                        ctx.moveTo(me.knee.x + 6, me.knee.y);
                        ctx.arc(me.knee.x, me.knee.y, 6, 0, 2 * Math.PI);
                        //foot joint
                        ctx.moveTo(me.foot.x + 5, me.foot.y);
                        ctx.arc(me.foot.x, me.foot.y + 1, 5, 0, 2 * Math.PI);
                        ctx.strokeStyle = "rgba(0,255,255,0.25)";
                        ctx.lineWidth = 6;
                        ctx.stroke();
                        ctx.fillStyle = me.fillColor;
                ...
                tungsten() {
                    ...
                    // me.setFillColors();
                    me.fillColor = `hsl(${me.color.hue},${me.color.sat}%,${me.color.light}%)`
                    me.fillColorDark = `hsl(${me.color.hue},${me.color.sat}%,${me.color.light - 50}%)`
                    ...
                    me.draw = function () {
                        ...
                        ctx.restore();
                        me.yOff = me.yOff * 0.9 + me.yOffGoal * 0.1; //smoothly move leg height towards height goal
                        powerUps.boost.draw()
                    }
                    me.drawLeg = function (stroke) {
                        // if (simulation.mouseInGame.x > me.pos.x) {
                        ...
                        //toe lines
                        ctx.beginPath();
                        ctx.moveTo(me.foot.x, me.foot.y - 1);
                        ctx.lineTo(me.foot.x - 15, me.foot.y + 5);
                        ctx.lineTo(me.foot.x + 15, me.foot.y + 5);
                        ctx.lineTo(me.foot.x, me.foot.y - 1);
                        ctx.lineWidth = 4;
                        ctx.stroke();

                        //hip joint
                        ctx.beginPath();
                        ctx.arc(me.hip.x, me.hip.y - 4, 12, 0, 2 * Math.PI);
                        //knee joint
                        ctx.moveTo(me.knee.x + 6, me.knee.y);
                        ctx.arc(me.knee.x, me.knee.y, 6, 0, 2 * Math.PI);
                        //foot joint
                ...
                anodize() {
                    ...
                    me.draw = function () {
                        if (powerUps.boost.endCycle > simulation.cycle) {
                            ...
                            ctx.beginPath();
                            const radius = 40
                            const mag = 12 * Vector.magnitude(me.velocitySmooth) + radius
                            ctx.arc(0, 0, radius, -Math.PI / 2, Math.PI / 2);
                            ctx.bezierCurveTo(-radius, radius, -radius, 0, -mag, 0); // bezierCurveTo(cp1x, cp1y, cp2x, cp2y, x, y)
                            ctx.bezierCurveTo(-radius, 0, -radius, -radius, 0, -radius);
                            // const time = (powerUps.boost.endCycle - me.cycle) / powerUps.boost.duration
                    ...
                    me.drawLeg = function (stroke) {
                        ...
                        //foot joint
                        ctx.beginPath();
                        ctx.arc(me.foot.x, me.foot.y, 4, 0, 2 * Math.PI);
                        ctx.fill();
                        ctx.restore();
                    }
                },
                dilate() {
                    ...
                    me.draw = function () {
                        const amplitude = 8 + 4 * Math.sin(me.cycle * 0.01)
                        ctx.fillStyle = me.fillColor;
                        me.walk_cycle += me.flipLegs * me.Vx;
                        ctx.save();
                        ctx.globalAlpha = (me.immuneCycle < me.cycle) ? 1 : me.cycle % 3 ? 0.1 : 0.65 + 0.1 * Math.random()
                        ...
                        ctx.beginPath();
                        ctx.arc(12, 0, amplitude, 0, 2 * Math.PI); //big eye
                        // ctx.fillStyle = `hsl(0,0%,${50+50*Math.sin(me.cycle * 0.0075+Math.PI)}%)` //`hsl(${150+50*Math.sin(me.cycle * 0.0075)},100%,50%)`
                        // ctx.fill();
                        ctx.stroke();
                        ctx.restore();
                        me.yOff = me.yOff * 0.85 + me.yOffGoal * 0.15; //smoothly move leg height towards height goal
                        ...
                        if (powerUps.boost.endCycle > simulation.cycle) {
                            ...
                            ctx.beginPath();
                            const radius = 39
                            const mag = 6 * Vector.magnitude(me.velocitySmooth) + radius
                            ctx.arc(0, 0, radius, -Math.PI / 2, Math.PI / 2);
                            ctx.bezierCurveTo(-radius, radius, -radius, 0, -mag, 0); // bezierCurveTo(cp1x, cp1y, cp2x, cp2y, x, y)
                            ctx.bezierCurveTo(-radius, 0, -radius, -radius, 0, -radius);
                        ...
                        simulation.setZoom(1800 + 500 * Math.sin(me.cycle * 0.01))
                    }
                },
                dilate2() {
                    me.isAltSkin = true
                    ...
                    me.draw = function () {
                        const amplitude = Math.sin(me.cycle * 0.01)
                        ...
                        ctx.fill();
                        ctx.strokeStyle = "#345";
                        ctx.lineWidth = 3 + 3 * Math.sin(me.cycle * 0.01 + Math.PI);
                        ctx.stroke();
                        ...
                        if (powerUps.boost.endCycle > simulation.cycle) {
                            ...
                            // ctx.lineWidth = (4 + 3 * Math.sin(me.cycle * 0.01 + Math.PI)) * time;
                            ctx.stroke();
                            ctx.restore();
                        }
                    ...
                    me.drawLeg = function (stroke) {
                        ...
                        ctx.lineTo(me.foot.x, me.foot.y);
                        ctx.strokeStyle = stroke;
                        ctx.lineWidth = 6 + 2 * Math.sin(me.cycle * 0.01 + Math.PI);
                        ctx.stroke();
                        ...
                        ctx.arc(me.hip.x, me.hip.y, 11, 0, 2 * Math.PI);
                        //knee joint
                        ctx.moveTo(me.knee.x + 7, me.knee.y);
                        ctx.arc(me.knee.x, me.knee.y, 7, 0, 2 * Math.PI);
                        //foot joint
                        ctx.moveTo(me.foot.x + 6, me.foot.y);
                        ctx.arc(me.foot.x, me.foot.y, 6, 0, 2 * Math.PI);
                        ctx.fillStyle = "#345";
                        ctx.fill();
                        ctx.lineWidth = 3 + 3 * Math.sin(me.cycle * 0.01 + Math.PI);
                        ctx.stroke();
                ...
                CPT() {
                    ...
                    me.draw = function () {
                        ...
                        ctx.beginPath();
                        ctx.arc(0, 0, 30, 0, 2 * Math.PI);
                        ctx.fillStyle = me.energy > 0.85 * Math.min(1, me.maxEnergy) ? me.bodyGradient : "#fff"
                        ctx.fill();
                        ctx.restore();

                        ctx.beginPath();
                        ctx.arc(me.pos.x + 15 * Math.cos(me.angle2), me.pos.y + 15 * Math.sin(me.angle2), 5, 0, 2 * Math.PI);
                        ctx.fillStyle = "#000"
                        ctx.fill();
                        me.yOff = me.yOff * 0.85 + me.yOffGoal * 0.15; //smoothly move leg height towards height goal
                        powerUps.boost.draw()
                ...
                verlet() {
                    ...
                    me.drawLeg = function (stroke) {
                        ...
                        ctx.beginPath();
                        ctx.moveTo(me.foot.x, me.foot.y);
                        const footDrop = me.onGround ? 5 : 10
                        ctx.lineTo(me.foot.x - 15, me.foot.y + footDrop);
                        ctx.moveTo(me.foot.x, me.foot.y);
                        ctx.lineTo(me.foot.x + 15, me.foot.y + footDrop);
                        ctx.lineWidth = 1;
                        ctx.stroke();
                ...
                stubs() {
                    ...
                    me.drawLeg = function (stroke) {
                        ...
                        ctx.moveTo(me.hip.x, me.hip.y);
                        ctx.lineTo(me.knee.x, me.knee.y);
                        ctx.lineTo(me.foot.x, me.foot.y + 5);
                        ctx.strokeStyle = stroke;
                        ctx.lineWidth = 6;
                        ctx.stroke();
                        ctx.restore();
                ...
                diegesis() {
                    ...
                    me.draw = function () {
                        ...
                        me.calcLeg(0, 0);
                        me.drawLeg("#333");
                        ctx.rotate(me.angle2 - (me.fireCDcycle !== Infinity ? me.flipLegs * 0.25 * Math.pow(Math.max(me.fireCDcycle - me.cycle, 0), 0.5) : 0));
                ...
                cat() {
                    ...
                    me.draw = function () {
                        ...
                        ctx.rotate(me.angle2);
                        if (!(me.angle2 > -Math.PI / 2 && me.angle2 < Math.PI / 2)) ctx.scale(1, -1);
                        ctx.beginPath();
                        ctx.moveTo(5, -30);
                        ctx.lineTo(20, -40);
                        ctx.lineTo(20, -20);
                ...
                pareidolia() {
                    ...
                    me.draw = function () {
                        ...
                        ctx.strokeStyle = "#333";
                        ctx.lineWidth = 2;
                        if (!(me.angle2 > -Math.PI / 2 && me.angle2 < Math.PI / 2)) ctx.scale(1, -1); //here is the flip
                        ctx.stroke();
                        ctx.beginPath();
            ...
            me.pushMobsFacing = function() { // find mobs in range and in direction looking
                ...
                if (Vector.magnitude(Vector.sub(m.pos, me.position)) - me.radius < me.fieldRange && Matter.Query.ray(map, m.pos, me.position).length === 0 && me.lookingAt(player)) {
			...
			me.do = function () {
                ...
                if (me.inputFire && (!me.inputField || me.fieldMode == 6 || me.fieldMode == 3 || me.fieldMode == 7 || me.fieldMode == 8 || me.fieldMode == 10) && me.cycle >= me.fireCDcycle) {
                    ...
                    if (me.gunType == 0 && me.cycle >= me.nextFireCycle) {//nailgun
                        ...
                        let direction = { x: Math.cos(me.angle2), y: Math.sin(me.angle2) }
